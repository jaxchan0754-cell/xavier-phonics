// phoneme 音素页：超大 grapheme + 锚定大图
// 自动慢速示范 1 遍（🔊 可重听）→ 跟读 → VAD 发声确认 → 通过后常速示范 1 遍作结
...
export function renderPhoneme(
  ...
  return new Promise((resolve) => {
    ...
    // 超大 grapheme + 锚定 emoji
    const big = el('div', 'phoneme-big')
    big.appendChild(el('span', 'phoneme-grapheme-big', activity.grapheme))
    big.appendChild(el('span', 'phoneme-emoji-big', activity.emoji))
